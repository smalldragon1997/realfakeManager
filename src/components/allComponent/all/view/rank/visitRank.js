...
class visitRank extends React.Component {
    ...
    render() {
        ...
        if (filter === "year") {
            ...
            // 过滤掉不在时间范围内的订单
            saleList = visitList.filter(item => (item.date >= rankStart && item.date < rankEnd));
            // 给坐标赋值
        ...
        saleList = visitList.filter(item => (item.date >= rankStart && item.date < rankEnd));
        // 获得所有出现过的商品
        const commList = saleList.reduce((commList, nextSaleList) => (commList.concat(nextSaleList)), []);
        // 给商品销量赋值
            ...
            if(commSaleData.length === 0){
                ...
                for (let j = 0; j < commSaleData.length; j++) {
                    if (commList[i].commId === commSaleData[j].commId) {
                        commSaleData[j].count+=commList[i].count;
                        flag = false;
                        break;
                    }
                }
            ...
            >
                <Row>
                    <Col xs={24} sm={24} md={24} lg={24} xl={18} xxl={18}>
                        <Row>
                            <Divider style={{marginTop:"1%",marginBottom:"1%"}}>浏览量趋势</Divider>
                        </Row>
                        {
                            filter === "year" ? (
                    ...
                    <Col xs={24} sm={24} md={24} lg={24} xl={6} xxl={6} style={{paddingLeft:"3%"}}>
                        <Row>
                            <Divider style={{marginTop:"1%",marginBottom:"1%"}}>商品浏览量排名</Divider>
                        </Row>
                        {
