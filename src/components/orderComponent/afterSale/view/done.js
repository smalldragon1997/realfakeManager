    ...
    render() {
        ...
        const columns = [
            {
                ...
                dataIndex: 'state',
                key: 'state',
                render:(state)=>(state===2?(<Tag color="blue">退款成功</Tag>):(<Tag color="red">退款失败</Tag>))
            }, {
                title: '操作',
                dataIndex: 'actions',
                key: 'actions',
                render: (afterSaleInfo) => (
                    ...
                    <Popconfirm placement="top" title={"确定删除此售后记录吗？"} onConfirm={() => {
                        let afterSaleIdList = [];
        ...
        for (let i = 0; i < afterSaleList.length; i++) {
            options.push({
                ...
                doneDate: new Date(afterSaleList[i].doneDate).Format("yyyy-MM-dd hh:mm:ss"),
                price: "￥" + afterSaleList[i].commodity.price,
                actions: afterSaleList[i],
                state: afterSaleList[i].state,
            })
        }
        return (
                                    ...
                                    <Row type={"flex"} align={"middle"}
                                        ...
                                        <Col span={4}>
                                            <Popconfirm placement="top"
                                                        title={"确定删除这" + selectedRowKeys.length + "个售后记录吗？"}
                                                        onConfirm={() => {
...
const mapStateToProps = (state) => {
    ...
    return {
        auth: navLink.auth,
        info: navLink.info,
        afterSaleList: getAfterSaleListByFilter(afterSale.afterSaleList, afterSale.filter, afterSale.key, afterSale.start, afterSale.end, 34, new Date()),
