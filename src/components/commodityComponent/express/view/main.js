...
class main extends React.Component {
        ...
        if (key === "") {
            client.search({
                ...
                function (body) {
                    ...
                    this.setState({expList: value.reduce((total, next) => (next._score === 0 ? (total) : (total.concat(next._source))), [])});
                }.bind(this),
                function (error) {
    ...
    render() {
        ...
        for (let i = 0; i < expList.length; i++) {
            options.push({
                key: expList[i].expId,
                expId: expList[i].expId,
                cover: expList[i].cover,
                price: expList[i].price,
                expName: expList[i].expName,
                actions: expList[i],
            })
        }
        return (
